...
type Props = typeof personStore.actionCreators &
  personStore.IPersonStoreState &
  RouteComponentProps<{}>;
...
class ExamplesPage extends React.Component<Props, IState> {
      ...
      <tr key={person.id}>
        ...
        <td>
          <button
            ...
            onClick={(x) => this.toggleUpdatePersonModal(person)}
          >
            Edit
          </button>
          &nbsp;
          <button
            ...
            onClick={(x) => this.toggleDeletePersonModal(person)}
          >
            Delete
          </button>
        </td>
  ...
  render() {
    return (
      <Container>
        <Helmet>
          <title>Example - ReactTypescriptBP</title>
        </Helmet>
          ...
          <Row>
            <div className="col-3 col-sm-2 col-md-2 col-lg-1">
              <button
                ...
                onClick={(x) => this.toggleAddPersonModal()}
              >
                Add
              </button>
            </div>
        ...
        <table className="table">
          ...
          <tbody>{this.renderRows(this.props.collection)}</tbody>
        </table>

        {/* Add modal */}
        <Modal
          show={this.state.isAddModalOpen}
          onHide={() => this.toggleAddPersonModal()}
        >
        ...
        <Modal
          ...
          onHide={() => this.toggleUpdatePersonModal()}
        >
          <Modal.Header closeButton>
            <Modal.Title>
              Edit person:{" "}
            ...
            {(renderEditor, handleSubmit) => (
              <>
                ...
                <Modal.Footer>
                  <Button
                    variant="secondary"
                    onClick={(x) => this.toggleUpdatePersonModal()}
                  >
                    Close
                  </Button>
                  <Button variant="primary" onClick={(x) => handleSubmit()}>
        ...
        <Modal
          ...
          onHide={() => this.toggleDeletePersonModal()}
        >
          <Modal.Header closeButton>
            <Modal.Title>
              Delete person:{" "}
          ...
          <Modal.Footer>
            <Button
              variant="secondary"
              onClick={(x) => this.toggleDeletePersonModal()}
            >
              Close
            </Button>
            <Button variant="primary" onClick={(x) => this.deletePerson()}>
              Save changes
            </Button>
          </Modal.Footer>
        </Modal>
